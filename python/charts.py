# ============================================================
# charts.py  —  4 charts telling the full retention story
# ============================================================
...
# ── CHART 1: AGAME Points Distribution by Status ────────────
ax1 = axes[0, 0]
...
ax1.set_xlabel('AGAME Points'); ax1.set_ylabel('Agent Count')
ax1.legend()
 
# ── CHART 2: TONE predicts RETENTION (scatter + trend) ──────
ax2 = axes[0, 1]
...
ax2.set_title('Tone (Sentiment) Predicts FCR\n— The Tone-Talent Signal', fontweight='bold', color=NAVY)
ax2.set_xlabel('Tone Sentiment Score'); ax2.set_ylabel('FCR %')
retained_patch = mpatches.Patch(color=TEAL, label='Retained')
risk_patch     = mpatches.Patch(color=RED,  label='At Risk')
ax2.legend(handles=[retained_patch, risk_patch, plt.Line2D([0],[0],color=GOLD,linestyle='--',label='Trend')])
 
# ── CHART 3: Retention Rate by Training Tier ────────────────
ax3 = axes[1, 0]
tier_order = ['High (12h+)', 'Mid (8-12h)', 'Low (<8h)']
df['Training_Tier'] = pd.Categorical(
    df['Training_Hours'].apply(lambda h: 'High (12h+)' if h>=12 else ('Mid (8-12h)' if h>=8 else 'Low (<8h)')),
    categories=tier_order, ordered=True)
tier_stats = df.groupby('Training_Tier', observed=True)['Retention_Status'].mean() * 100
...
ax3.set_title('Retention Rate by Training Tier\nMore Training = Less Attrition Risk', fontweight='bold', color=NAVY)
ax3.set_ylabel('Retention Rate %'); ax3.set_ylim(0, 105)
 
# ── CHART 4: Composite Engagement Score ─────────────────────
ax4 = axes[1, 1]
...
ax4.set_title('Composite Engagement Score\nThe 15% Attrition Risk Reduction Signal', fontweight='bold', color=NAVY)
ax4.set_xlabel('Engagement Score'); ax4.set_ylabel('Agent Count')
ax4.legend()
